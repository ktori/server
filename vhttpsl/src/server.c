...
vhttpsl_server_listen_http(vhttpsl_server_t server, int port)
{
	...
	event.data.ptr = socket_context_create(socket_fd, SCT_SERVER_SOCKET);
...
vhttpsl_server_poll(vhttpsl_server_t server)
{
	...
	{
		event_fd = ((socket_context_t) events[i].data.ptr)->fd;
		...
		if (event_fd == server->socket_fd)
		{
			...
			event.data.ptr = socket_context_create(client_fd, SCT_CLIENT_SOCKET);
			event.events = EPOLLIN;
		...
		else
		{
			...
			read_ln = read(event_fd, buffer, sizeof(buffer));

			if (read_ln < 0)
			{
				perror("read()");

				socket_context_destroy(events[i].data.ptr);
				close(event_fd);
			...
			fprintf(stdout, "read %d bytes from the client\n", read_ln);
